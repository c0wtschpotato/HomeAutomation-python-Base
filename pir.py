...
def hotornot():
    if datetime.datetime.utcnow() > astral.sun.sun.night(obs)[1].replace(tzinfo=None) is True and datetime.datetime.utcnow() < astral.sun.sun.night(obs)[0].replace(tzinfo=None) is True:
        print("Es ist Nacht")
        return True
